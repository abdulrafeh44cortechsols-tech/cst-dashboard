...
import { X, Plus, Trash2, Loader2, ImageIcon, Upload, CheckCircle2, XCircle } from "lucide-react";
...
export function EditProjectForm({ project, onCancel, onSaved }: EditProjectFormProps) {
  ...
  const [projectGoalContent, setProjectGoalContent] = useState<ProjectGoalContentItem[]>([]);
  const [projectGoalSectionIntro, setProjectGoalSectionIntro] = useState("");
  const [projectGoalSectionItems, setProjectGoalSectionItems] = useState<ProjectGoalSectionItem[]>([]);

  // Technology section
  const [technologyTitle, setTechnologyTitle] = useState("");
  const [technologyDescription, setTechnologyDescription] = useState("");
  const [technologyContent, setTechnologyContent] = useState<TechnologyContentItem[]>([]);
  ...
  useEffect(() => {
    if (project) {
      ...
      if (projectData.tags && Array.isArray(projectData.tags) && tagsResponse?.results) {
        const tagIds = projectData.tags
          .map((tagName: string) => {
            const tag = tagsResponse.results.find((t: Tag) => t.name === tagName);
            return tag?.id;
          })
      ...
      if (projectData.project_goal_section) {
        setProjectGoalSectionIntro(projectData.project_goal_section.intro || "");
        setProjectGoalSectionItems(projectData.project_goal_section.items || []);
      }
  ...
  const handleHeroImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      toast.info('Please add alt text before uploading the image', { duration: 4000 });
    }
  };
  ...
  const handleIconFilesChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    toast.info(`${files.length} icon(s) selected. Please add alt text for each image.`, { duration: 4000 });
  };
  ...
  const uploadIcons = async () => {
    ...
    const emptyAltTexts = iconAltTexts.some((alt, index) => index < iconFiles.length && !alt.trim());
    if (emptyAltTexts) {
  ...
  const handleTechFilesChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    toast.info(`${files.length} technology image(s) selected. Please add names and alt text for each.`, { duration: 4000 });
  };
  ...
  const uploadTechImages = async () => {
    ...
    try {
      ...
      toast.success(`${uploadedImages.length} technology image(s) uploaded successfully!`);
      ...
      toast.error("Failed to upload technology images: " + (error.message || "Unknown error"));
    } finally {
  ...
  const uploadTechIcon = async (index: number) => {
    ...
    try {
      ...
      const uploadedImages = await mediaService.bulkUploadMedia(item.iconFiles, item.iconAltTexts);
  ...
  const updatePoint = (itemIndex: number, pointIndex: number, value: string) => {
    const updated = [...projectGoalSectionItems];
    if (!updated[itemIndex].points) {
      updated[itemIndex].points = [];
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updateData = {
        ...
        icons: icons.map(icon => icon.id).filter((id): id is number => id !== undefined),
      ...
      console.log("DEBUG - Icons being sent:", icons.map(icon => icon.id).filter((id): id is number => id !== undefined));
              ...
              <div className="space-y-2">
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <Input
                        ...
                        maxLength={255}
                        disabled={uploadingHeroImage || (!!heroImageId && !heroImageFile)}
                      />
                    ...
                    {heroImageFile && !heroImageId ? (
                      <Button
                        ...
                        disabled={!heroImageAltText.trim() || uploadingHeroImage}
                        size="sm"
                        className="w-full"
                      >
                        {uploadingHeroImage ? (
                      ...
                      <div className="flex items-center gap-2 text-sm text-green-600">
                        ...
                        {heroImageFile ? "New image uploaded successfully!" : "Image already uploaded"}
                      </div>
                    ) : null}
                  </div>
                )}
                ...
                {heroFromGallery && heroImageId && (
                  <div className="flex items-center gap-2 text-sm text-green-600">
                    ...
                    Image selected from gallery (Alt: {heroImageAltText || 'No alt text'})
                  </div>
                )}
              </div>
        ...
        <TabsContent value="challenge" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <p className="text-sm text-muted-foreground">
                Describe the main challenge or problem that this project addressed
              </p>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                  ...
                  onChange={(e) => {
                    if (e.target.value.length <= 1000) {
                      ...
                      if (e.target.value.trim().length > 0 && e.target.value.trim().length < 100) {
                        setHeroDescriptionError("Challenge description must be at least 100 characters long.");
                      } else {
                ...
                <p className="text-sm text-muted-foreground">
                  {challengeDescription.length}/1000 characters (minimum 100 required)
                </p>
              </div>
            </CardContent>
          </Card>
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <Button
                    ...
                    size="sm"
                    onClick={() => {
                      setProjectGoalContent([...projectGoalContent, { goal: "", icon: "", alt_text: "" }]);
                    }}
                  >
                          ...
                          <div className="p-2 border rounded bg-gray-50">
                            ...
                            {item.iconFile && !item.uploaded && (
                              <Button
                                ...
                                disabled={!item.alt_text.trim() || item.uploading}
                                size="sm"
                  ...
                  <div key={index} className="p-4 border rounded-lg space-y-3">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="space-y-2">
                          {(item.points || []).map((point: string, pointIndex: number, arr: any) => {
                                  ...
                                  <div className="flex gap-2 items-center">
                                    <Input
                                      ...
                                      onChange={(e) => {
                                        const value = e.target.value;
                                        if (value.length <= 200) {
                                          updatePoint(index, pointIndex, value);
                                        }
                                      }}
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            ...
                            updated[index] = {
                              ...
                              icons: files.map(file => ({ url: URL.createObjectURL(file), alt_text: '' })), // Preview URLs
                              uploaded: false,
                              uploading: false
                            };
                            setTechnologyContent(updated);
